...
# makes a list with all combinations of the parameters and put them in a list
# (for advent07 relevant - all combinatins as input for the amplifiers)
def make_combi(numb):
    combi2 = []
...
# initializing the list from x to the maximum value
# extension of the list - when the adress targeting is outside the actual list borders
def memory_extend(list_extend_index):
    for j in range(list_extend_index - len(liste) + 2):
        liste.append(0)

# checks if a value is over the end of the list
# relevant for mode 2 implementation
...
# logic for robo movment of Advent 11 challenge
...
# OPCODE 1 - add 2 values according to the position
# 1st result, 2nd value1, 3rd value2
...
# OPCODE 2 - multiplicating 2 values according to the position
# 1st result, 2nd value1, 3rd value2
...
# OPCODE 3 - input of a value according to the position
# with 0 input according to  position
# with 1 input directly on the actual position
# I1 or I2 - writing the code according to the position
def op_in(inst, code):
    global i
