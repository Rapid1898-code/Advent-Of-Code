    ...
    for idx, entry in enumerate(c_sue):
        ...
        if idx % 2 == 0:
            ...
            for idx2, entry2 in enumerate(target):
                check = False
                if idx2 % 2 == 0:
                    if check_name == target[idx2] in ['cats','trees']:
                        if check_anz > target[idx2 + 1]:
                            check = True
                            break
                    if check_name == target[idx2] in ['pomeranians', 'goldfish']:
                        if check_anz < target[idx2 + 1]:
                            check = True
                            break
                    if check_name == target[idx2] not in ['pomeranians', 'goldfish', 'cats','trees']:
                        if check_anz == target[idx2 + 1]:
                            check = True
                            break
            if check == False:
