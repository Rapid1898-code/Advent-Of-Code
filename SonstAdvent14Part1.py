...
for i in result:
    idx = find_element(ore, i[0])
    idx_rest = find_rest(rest, i[0])
    if idx_rest == None: rest_diff = 0
    else: rest_diff = rest[idx_rest][1]

    tmp_calc1 = math.ceil(i[1] / ore[idx][1])     # aufrundung ohne berückischtigung rest
    tmp_calc2 = tmp_calc1 * ore[idx][1]     # Ergebnis mit Aufrundung ohne Rest

    if rest_diff > tmp_calc2 - i[1]:     # Rest reicht aus
        calc = (tmp_calc2 - i[1]) * i[1]
    else:
        calc = tmp_calc1 * ore[idx][0][0]
    # print(i, calc)
    result_end += calc
